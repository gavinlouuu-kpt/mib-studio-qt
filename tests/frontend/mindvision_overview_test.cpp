...
int main(int argc, char** argv) {
    ...
    backend.setMindVisionCameraSelection(0, "Test camera");
    {
        ...
        auto* preview = window.findChild<frontend::PreviewPage*>();
        MIB_REQUIRE(preview, "experiment preview exists");
        auto* panel = preview->getPlaybackPanel();
        panel->setRoi(QRect(282, 5, 225, 82), false);
        MIB_REQUIRE(
        ...
        MIB_EXPECT(panel->getRoi() == QRect(0, 0, 640, 96),
                   "Experiment overlay covers the Overview-selected hardware crop");
        const auto previewRoi = backend.processing().getRealtimeRoi();
        MIB_EXPECT(previewRoi.x == panel->getRoi().x() && previewRoi.y == panel->getRoi().y() &&
                       previewRoi.w == panel->getRoi().width() &&
                       previewRoi.h == panel->getRoi().height(),
                   "Experiment overlay and realtime processing use the same crop-local ROI");
        ...
        // Exercise the Euresys frontend route without opening a physical device.
        // Invalid selection indices keep script application behind its guards.
        backend.setHardwareCameraSelection(-1, -1, "Euresys ROI compatibility");
        MIB_REQUIRE(!backend.isMindVisionCameraSelected(), "Euresys leaves MindVision mode");
        std::ofstream(js) << "g.RemotePort.set(\"Width\", 512);\n"
                             "g.RemotePort.set(\"Height\", 96);\n"
                             "g.RemotePort.set(\"OffsetX\", 704);\n"
                             "g.RemotePort.set(\"OffsetY\", 500);\n";
        auto* euresysOverview = window.findChild<frontend::OverviewTab*>();
        MIB_REQUIRE(euresysOverview, "Overview exists for provider switching");
        euresysOverview->refreshCameraMode();
        MIB_EXPECT(euresysOverview->roiPosition() == QPointF(704, 500),
                   "Euresys loads sensor offsets from its JS profile");
        MIB_REQUIRE(QMetaObject::invokeMethod(euresysOverview, "onRoiPositionChanged",
                                              Qt::DirectConnection,
                                              Q_ARG(QPointF, QPointF(720, 504))),
                    "move Euresys Overview ROI");
        const auto euresysRoi = backend.processing().getRealtimeRoi();
        MIB_EXPECT(euresysRoi.x == 720 && euresysRoi.y == 504,
                   "Euresys retains sensor offsets rather than MindVision crop-local origin");
        int savedX = 0, savedY = 0;
        MIB_REQUIRE(frontend::EgrabberConfigParser::readRoiOffsets(
                        QString::fromStdString(js.string()), savedX, savedY),
                    "read Euresys ROI profile");
        MIB_EXPECT(savedX == 720 && savedY == 504, "Euresys ROI edit persists to JS");
        const QRect subRoi(24, 8, 160, 64);
        panel->setRoi(subRoi, false);
        MIB_REQUIRE(
            QMetaObject::invokeMethod(&window, "onTabChanged", Qt::DirectConnection, Q_ARG(int, 2)),
            "navigate Euresys Experiment");
        MIB_EXPECT(panel->getRoi() == subRoi,
                   "MindVision synchronization does not reset Euresys preview ROI");
        MIB_EXPECT(nlohmann::json::parse(std::ifstream(path)).at("offset_x") == 64,
                   "Euresys edits leave MindVision profile untouched");
    }
    backend.setMindVisionCameraSelection(0, "Test camera");
    MIB_REQUIRE(backend.stageMindVisionConfigFromFile(path.string(), &error), error.c_str());
