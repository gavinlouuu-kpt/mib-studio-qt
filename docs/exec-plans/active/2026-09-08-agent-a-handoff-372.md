...
| G8 | The Rust bridge only builds on Linux (`build.rs` hard-codes the `linux-backend-only` preset and Ubuntu library paths). | M4 requires native Windows evidence. | Prototype on `agent-b/windows-native-bridge` (local, this bench): a CMake-derived link manifest (`tools/gen_bridge_link_manifest.py`) and a Windows path in `build.rs`. Owner: Agent B; Agent A provides the manifest generator. |
