...
| G7 | `dev/react-tauri` is 77 commits behind `develop`; trial merge of today's `develop` (without the coordinator) into Agent B's tip conflicts in 18 files (`CameraControlService.cpp`, `CrashReporter.cpp`, `SyringePumpService.{h,cpp}`, `TriggerService.cpp`, `MindVisionConfig.h`, `MockCamera.h`, CI yml, vault). | Every one of these is a reliability or camera fix that must win. | Agent B rebases its stack onto `develop` after #379 merges; Agent A reviews the six backend conflicts. **Backend part done on `agent-b/shared-backend-integration` (PR #380):** the reliability serial bus (`SerialBus`, `SyringePumpService`, `PulseGeneratorService`, `ModbusRtu.h`) is ported onto `ISerialPort` (line settings, named open, OS error code, Qt-free port enumeration), the crash reporter's Qt message handler stays in the frontend (`QtLogBridge`), and `mib_backend` links no Qt. Guards: `backend.syringe_pump_fake_serial`, `backend.pump_bridge_facade`, `backend.pulse_generator_frame`, `backend.modbus_rtu`, bridge crate 16/16; `backend.serial_bus_pty` (POSIX) is ported but only compiles on Linux CI. |
