...
- [x] 2026-09-16: PR #421 CI fully green — 16/16 lanes passed (Linux backend, TSan, ASan/UBSan, Rust bridge, React+Tauri desktop, Python wheels x86_64+aarch64, Windows native core, knowledge base, validate). Five CI-fix commits: GCC aggregate-init overload (`9dfee2a`), missing `<stdexcept>` in `fake_discovery_providers.h` and `device_discovery_providers_test.cpp`, Rust contract test accepting `Failed` state on SDK-less platforms.
