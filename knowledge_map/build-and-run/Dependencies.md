...
| zlib | `[>=1.2.11 <2]` (resolves 1.3.2 fresh; 1.3.1 on the rig PC's cache) | | **Direct** requirement since 2026-09-26 (ADR 0006: the compression writer calls zlib itself). Same range the hdf5 recipe declares, so it resolves to the package already in the graph (checked for `linux-gcc13` and `windows-msvc194-ninja`: one host zlib, 1.3.2). A machine with an older cache keeps whatever zlib its HDF5 already uses (the rig PC: 1.3.1). Either way, no new binaries are needed. An old `build-ninja` Conan output from before this change still configures, but compiling fails with `zlib.h` not found; re-run `conan install`. `cmake/MIBDependencies.cmake` does `find_package(ZLIB REQUIRED)`: configure fails if absent. System equivalents: apt `zlib1g-dev`, manylinux `zlib-devel`, macOS SDK `libz`. |
