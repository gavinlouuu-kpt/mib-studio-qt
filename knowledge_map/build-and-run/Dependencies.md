...
| zlib | `[>=1.2.11 <2]` (resolves 1.3.2) | | **Direct** requirement since 2026-09-26 (ADR 0006: the compression writer calls zlib itself). Same range the hdf5 recipe declares, so it resolves to the package already in the graph (checked for `linux-gcc13` and `windows-msvc194-ninja`: one host zlib, 1.3.2). `cmake/MIBDependencies.cmake` does `find_package(ZLIB REQUIRED)`: configure fails if absent. System equivalents: apt `zlib1g-dev`, manylinux `zlib-devel`, macOS SDK `libz`. |
