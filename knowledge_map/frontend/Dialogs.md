...
| `MonitoringSettingsDialog` | Scatter density (KDE) bandwidth factor, update interval and core contour percentage; reference contour actions (*Pin current* / *From file…* / *Clear*, immediate, not applied; *From file…* shows a warning naming why a file has no usable contour); fixed scatter/histogram axis ranges, histogram bin width. The KDE values persist via the tab (`QSettings` `Monitoring/Kde*`); the axis ranges do not. | [[ExperimentMonitoringTab]] |
