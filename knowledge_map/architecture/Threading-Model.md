...
| Dot-grid localization | [[../services/DotGridService]] `loop()` | `wakeCv_` + `interval_ms` timeout | Samples the latest committed FrameStore frame at a low rate, decodes the wafer fiducial pattern, publishes a pose snapshot + callback; never on the capture/realtime threads |
