...
- **Beta channel unblocked: 1.0.7 version sync + deterministic
  ...
  1.0.3 → 1.0.7, manual-screenshot manifest refresh), making `develop`
  resolve 1.0.7 so auto-betas stop tagging `v1.0.6-beta.*`. Fixed the
  `camera.delivery_mode_overload` gate that failed every beta build since
  #336: overload was generated by sleep-rate ratios that Windows CI's
  ~15.6 ms timer granularity flattens to 1:1. The test now paces the
  consumer on logical producer progress, `queue_camera.h` gained a
  `produceBurst` option (used by `camera.delivery_mode_contract`), and
  freshness/frame-age gates compare medians and cross-mode bounds instead
  of per-sample maxima. Details in
  [[../task/2026-08-01-frame-delivery-mode]].
