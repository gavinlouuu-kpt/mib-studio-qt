...
beforeEach(()=>{vi.resetAllMocks();vi.mocked(bridge.isInitialized).mockResolvedValue(true);vi.mocked(invoke).mockResolvedValue({capture_running:true,recording:true});vi.mocked(bridge.fetchExperimentStatus).mockResolvedValue({state:2} as never);vi.mocked(bridge.fetchReviewMetadata).mockResolvedValue({file_open:true,recording_file:true,file_path:"retained.h5"} as never);});
it("resumes native capture, recording, experiment and open review without initialization",async()=>{const value=await recoverNativeRuntime();expect(value).toMatchObject({resumed:true,runtime:{capture_running:true,recording:true},experiment:{state:2},review:{file_path:"retained.h5"}});expect(bridge.init).not.toHaveBeenCalled();});
