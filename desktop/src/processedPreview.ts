...
export interface ProcessedPreview {
  ...
  processing_session: string; store_generation: string; recipe_sha256: string;
...
export function decodeProcessedPreview(buffer: ArrayBuffer): ProcessedPreview | null {
  ...
  for (const name of ['frame_index', 'source_timestamp', 'host_timestamp_us', 'processing_session', 'store_generation']) {if (typeof meta[name] !== 'string') throw new Error('Invalid processed preview identity'); decimalU64(meta[name]);}
  ...
  for (const key of ['contours_truncated', 'primary_object_valid', 'primary_object_target']) if (typeof meta[key] !== 'boolean') throw new Error('Invalid preview flags');
