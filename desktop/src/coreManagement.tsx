...
export function useCoreManagement({ready,active,resume=false,append,onChanged}:{ready:boolean;active:boolean;resume?:boolean;append:(s:string)=>void;onChanged:()=>Promise<void>}){
 ...
 const run=async(action:"restore"|"check"|"activate"|"bundled")=>{
  if(!ready||active||appUpdateBusy||pending.current)return;pending.current=true;setBusy(true);setMessage("");
  try{
   if(action==="check"){
 ...
 return {info,initialized,appUpdateBusy,setAppUpdateBusy,updateBlocked:!ready||active||busy,report:(s:string)=>setMessage(s),busy:busy||appUpdateBusy,message,channel,setChannel:(c:string)=>{setChannel(c);setIndex(null);},registry,setRegistry:(s:string)=>{setRegistry(s);setIndex(null);},index,selected,setSelected,blocked:!ready||active||busy||appUpdateBusy,run};
}
export function CoreManagementPanel({model:m,updatesBlocked=false}:{model:ReturnType<typeof useCoreManagement>;updatesBlocked?:boolean}){
 ...
 <ApplicationUpdateControls channel={m.channel} blocked={m.updateBlocked||updatesBlocked} onBusy={m.setAppUpdateBusy}/>
