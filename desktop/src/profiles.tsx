...
export interface Profile { name:string; path:string; revision:string; document_json?:string; script?:string|null }
interface Reply { ok:boolean; error?:string; profiles?:Profile[]; profile?:Profile; destination?:string; warnings?:string[]; applied?:boolean; message?:string; display_fps?:number; profile_id?:string }
export const profileCommand = (base:string, request:object) => invoke<Reply>("profile_command",{base,request:JSON.stringify(request)});
export function useProfiles({ready,active,append,onOpen,onApplied}:{ready:boolean;active:boolean;append:(s:string)=>void;onOpen:(path:string)=>Promise<void>;onApplied?:()=>Promise<void>}) {
  ...
  const run=async(operation:string, target?:Profile)=>{
    ...
    try {
      ...
      const r=await profileCommand(directory,q);if(!r.ok)throw new Error(r.error);
  ...
  return {base,profiles,selected,name,setName,document,edit:(v:string)=>{setDocument(v);setDirty(true);},script,editScript:(v:string|null)=>{setScript(v);setDirty(true);},dirty,busy,blocked:!ready||active||busy,message,run};
}
export function ProfilesPanel({model:m}:{model:ReturnType<typeof useProfiles>}) {
  return <section aria-label="Local profiles"><h5>Local configuration profiles</h5>
    ...
    <p className="mono">{m.base||"No profile folder selected"}</p>
