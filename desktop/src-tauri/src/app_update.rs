...
mod tests {
 ...
 #[test]fn active_or_unknown_native_state_blocks_install(){let idle_job=serde_json::json!({"state":"idle"});assert!(idle(true,0,Some(false),Some(false),std::slice::from_ref(&idle_job)).is_ok());assert!(idle(true,2,Some(false),Some(false),std::slice::from_ref(&idle_job)).is_err());assert!(idle(true,0,Some(true),Some(false),std::slice::from_ref(&idle_job)).is_err());assert!(idle(true,0,Some(false),Some(true),std::slice::from_ref(&idle_job)).is_err());assert!(idle(false,0,Some(false),Some(false),&[idle_job]).is_err());assert!(idle(true,0,Some(false),Some(false),&[serde_json::json!({"state":"running"})]).is_err());assert!(idle(true,0,None,Some(false),&[]).is_err());}
