...
fn require_idle(app:&AppHandle)->Result<String,String>{
 let state=app.state::<AppState>();let mut bridge=state.bridge.lock().map_err(|e|e.to_string())?;
 require_idle_locked(&mut bridge)
}
fn require_idle_locked(bridge:&mut cxx::UniquePtr<mib_bridge::ffi::BackendBridge>)->Result<String,String>{
 ...
 let info:serde_json::Value=serde_json::from_str(&bridge.pin_mut().processing_core_command("",r#"{"operation":"info"}"#)).map_err(|e|e.to_string())?;
 Ok(info["app_version"].as_str().ok_or("Cannot verify installed version")?.to_owned())
}
...
pub async fn check_tauri_app_update(app:AppHandle,channel:String)->Result<CheckedRelease,String>{
 ...
 let current=require_idle(&app)?;let os=std::env::consts::OS;let arch=std::env::consts::ARCH;
...
pub async fn verify_tauri_app_installer(app:AppHandle,token:String,path:String,ui_idle:bool)->Result<(),String>{
 ...
 tauri::async_runtime::spawn_blocking(move||{
 let current=require_idle(&app)?;let mut state=tickets().lock().map_err(|e|e.to_string())?;let ticket=state.as_mut().filter(|t|t.token==token).ok_or("Update check is no longer current")?;fresh(ticket)?;
...
pub async fn launch_tauri_app_installer(app:AppHandle,token:String,ui_idle:bool,confirmed:bool)->Result<(),String>{
 ...
 tauri::async_runtime::spawn_blocking(move||{
 let current=require_idle(&app)?;let mut state=tickets().lock().map_err(|e|e.to_string())?;let ticket=state.as_ref().filter(|t|t.token==token).ok_or("Update check is no longer current")?;
 let latest=validate(&fetch(&ticket.feed)?,&current,std::env::consts::OS,std::env::consts::ARCH)?;same_release(ticket,&latest)?;
